...
const SELECT_KEY_FIELDS: &str = "SELECT fingerprint, key_type, name, public_key, encrypted_private_key, created_at, updated_at FROM keys";
...
pub async fn get_keys_by_type(pool: &SqlitePool, key_type: &str) -> Result<Vec<Key>> {
    let keys = sqlx::query_as::<_, Key>(&format!(
        "{SELECT_KEY_FIELDS} WHERE key_type = ? ORDER BY name"
    ))
    .bind(key_type)
    .fetch_all(pool)
    .await?;
...
pub async fn get_key(pool: &SqlitePool, fingerprint: &str) -> Result<Option<Key>> {
    let key = sqlx::query_as::<_, Key>(&format!("{SELECT_KEY_FIELDS} WHERE fingerprint = ?"))
        .bind(fingerprint)
        .fetch_optional(pool)
        .await?;
...
pub async fn get_vault_key(pool: &SqlitePool, fingerprint: &str) -> Result<Option<Key>> {
    let key = sqlx::query_as::<_, Key>(&format!(
        "{SELECT_KEY_FIELDS} WHERE fingerprint = ? AND key_type = 'vault'"
    ))
    .bind(fingerprint)
    .fetch_optional(pool)
    .await?;
...
pub async fn get_host_key_by_fingerprint(
    ...
    let key = sqlx::query_as::<_, Key>(&format!(
        "{SELECT_KEY_FIELDS} WHERE fingerprint = ? AND key_type = 'host'"
    ))
    .bind(fingerprint)
    .fetch_optional(pool)
    .await?;
